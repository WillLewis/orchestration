...
If a referenced file is missing, note it in your final handoff and continue with the available
context.
...
- Do not implement retrieval, guard, prompt/runtime, telemetry, or frontend changes unless the user
  redirects this thread.
...
4. Maintain demo readiness for the refusal, sealed-record, restricted-source, unrelated/no-results,
   toggle, and unavailable-path cases.
5. Keep the integration order current.

## Acceptance Criteria

- The plan and workstreams docs are internally consistent.
- Every workstream has a clear owner, scope, out-of-scope list, acceptance criteria, and
