        ...
        <main className="w-full min-w-0 space-y-7 md:flex-1">
          ...
          <Block title="What fundamentally changes for the user">
            <p>
              ...
              approvals, chasing owners, documenting what was decided. The agent describes; she does
              the rest.
            </p>
            <p>
              The enhanced agent collapses that gap. Because it can both see the true state across
              her sources and act on it, the work between the meeting and the outcome happens inside
              one governed thread — it drafts the decision, proposes the follow-ups, and routes
              them. Dana approves the consequential steps instead of performing all of them. The
              improvement isn't a smarter answer; it's that the agent absorbs the work between the
              decision and the done, and she stops being the integration layer between her own
              tools.
            </p>
          </Block>
          ...
          <Block title="What happens when grounding meets action">
            <p>
              ...
              engine — it knows the state but can't change it. Safe actions by themselves are a tool
              executor — they can change things but don't know what's true. Fused, the agent can see
              what's true and what's missing, and resolve the gap within policy.
            </p>
            <p>
              It also closes a longer loop than a single meeting. Because the agent reads from live
              sources and writes governed records, it notices when a sealed decision goes stale and
              re-validates it. A read-only agent hands you an answer that rots silently; an agent
              that reads and writes stays accountable for whether "done" is still true.
            </p>
          </Block>
          ...
          <Block title="Why this raises the stakes — and the answer">
            <p>
              The same fusion that creates the value is why governance can't be an afterthought. The
              moment an agent that reads untrusted content can also write, a wrong read — or an
              instruction injected into a document — becomes an action, not just a bad sentence.
              Power and exposure rise together.
            </p>
            <p>
